...
def rotate(image, radar_images, annotations, prob=0.5, border_value=(128, 128, 128)):
    ...
    radar_images = [cv2.warpAffine(ri, M=M, dsize=(new_w, new_h), flags=cv2.INTER_CUBIC,
                           borderMode=cv2.BORDER_CONSTANT,
                           borderValue=border_value) for ri in radar_images]
...
def crop(image, radar_images, annotations, prob=0.5):
    assert 'bboxes' in annotations, 'annotations should contain bboxes even if it is empty'
    ...
    if bboxes.shape[0] != 0:
        ...
        radar_images = [ri[random_y1:random_y2, random_x1:random_x2] for ri in radar_images]
...
def flipx(image, radar_images, annotations, prob=0.5):
    assert 'bboxes' in annotations, 'annotations should contain bboxes even if it is empty'
    ...
    h, w = image.shape[:2]
    image = image[:, ::-1]
    radar_images = [ri[:, ::-1] for ri in radar_images]
    if bboxes.shape[0] != 0:
        tmp = bboxes.copy()
...
def multi_scale(image, radar_images, annotations, prob=1.):
    assert 'bboxes' in annotations, 'annotations should contain bboxes even if it is empty'
    ...
    radar_images = [cv2.resize(ri, (nw, nh), interpolation=cv2.INTER_LINEAR) for ri in radar_images]
    bboxes = annotations['bboxes']
    if bboxes.shape[0] != 0:
...
def translate(image, radar_images, annotations, prob=0.5, border_value=(128, 128, 128)):
    assert 'bboxes' in annotations, 'annotations should contain bboxes even if it is empty'
...
class MiscEffect:
    ...
    def __call__(self, image, radar_images, annotations):
        image, radar_images, annotations = multi_scale(image, radar_images, annotations, prob=self.multi_scale_prob) # done
        image, radar_images, annotations = rotate(image, radar_images, annotations, prob=self.rotate_prob, border_value=self.border_value) # done
        image, radar_images, annotations = flipx(image, radar_images, annotations, prob=self.flip_prob) # done
        image, radar_images, annotations = crop(image, radar_images, annotations, prob=self.crop_prob) # done
        image, radar_images, annotations = translate(image, radar_images, annotations, prob=self.translate_prob, border_value=self.border_value) # done
        return image, radar_images, annotations

...
class Generator(keras.utils.Sequence):
    ...
    def load_radar_images(self, image_index):
        """
        Load an image at the image_index.
        """
        raise NotImplementedError('load_image method not implemented')
    ...
    def load_radar_group(self, group):
        """
        Load images for all images in a group.
        """
        return [self.load_radar_images(image_index) for image_index in group]
    ...
    def random_misc_group_entry(self, image, radar_images, annotations):
        """
        Randomly transforms image and annotation.
        """
        # randomly transform both image and annotations
        image, radar_images, annotations = self.misc_effect(image, radar_images, annotations)
        return image, radar_images, annotations

    def random_misc_group(self, image_group, radar_group, annotations_group):
        """
        ...
        if self.misc_effect is None:
            return image_group, radar_group, annotations_group

        for index in range(len(image_group)):
            # transform a single group entry
            image_group[index], radar_group[index], annotations_group[index] = self.random_misc_group_entry(
                image_group[index], radar_group[index], annotations_group[index])

        return image_group, radar_group, annotations_group

    def preprocess_group_entry(self, image, radar_images, annotations):
        """
        Preprocess image and its annotations.
        """

        # preprocess the images
        image, radar_images, scale = self.preprocess_image(image, radar_images)
    ...
    def preprocess_group(self, image_group, radar_group, annotations_group):
        """
        ...
        for index in range(len(image_group)):
            ...
            image_group[index], radar_group[index], annotations_group[index] = \
                self.preprocess_group_entry(image_group[index], radar_group[index], annotations_group[index])
    ...
    def compute_inputs(self, image_group, radar_group, annotations_group):
        """
        Compute inputs for the network using an image_group.
        """
        batch_images = np.array(image_group).astype(np.float32)
        # batch_radar_images = [np.array([rg[i] for rg in radar_group]).astype(np.float32) for i in range(5])
        batch_radar_P3_images = np.array([rg[0] for rg in radar_group]).astype(np.float32)
        batch_radar_P4_images = np.array([rg[1] for rg in radar_group]).astype(np.float32)
        batch_radar_P5_images = np.array([rg[2] for rg in radar_group]).astype(np.float32)
        batch_radar_P6_images = np.array([rg[3] for rg in radar_group]).astype(np.float32)
        batch_radar_P7_images = np.array([rg[4] for rg in radar_group]).astype(np.float32)


        return [batch_images, batch_radar_P3_images, batch_radar_P4_images, batch_radar_P5_images,
                batch_radar_P6_images, batch_radar_P7_images]
    ...
    def debug_images_fn(self, image_group, radar_group):
        debug_images = []
        for image, radar_images in zip(image_group, radar_group):
            radar_image = radar_images[0]
            for ri in radar_images[1:]:
                radar_image = cv2.add(radar_image, ri)
            debug_images.append(cv2.add(image, radar_image))
    ...
    def compute_inputs_targets(self, group, debug=False):
        ...
        radar_group = self.load_radar_group(group)
        ...
        image_group, radar_group, annotations_group = self.random_misc_group(image_group, radar_group,
                                                                             annotations_group)

        # perform preprocessing steps
        image_group, radar_group, annotations_group = self.preprocess_group(image_group, radar_group, annotations_group)
        ...
        inputs = self.compute_inputs(image_group, radar_group, annotations_group)
    ...
    def preprocess_image(self, image, radar_images):
        # image, RGB
        ...
        radar_images = [cv2.resize(ri, (resized_width, resized_height)) for ri in radar_images]
        image = image.astype(np.float32)
        radar_images = [ri.astype(np.float32) for ri in radar_images]
        image /= 255.
        radar_images = [ri/255. for ri in radar_images]
        ...
        radar_images = [np.pad(ri, [(0, pad_h), (0, pad_w)], mode='constant') for ri in radar_images]
        # radar_images = [ri.reshape(ri.shape[0], ri.shape[1], 1) for ri in radar_images]
        # radar_images = [np.tile(ri, (1, 1, filter_depth)) for ri, filter_depth in zip(radar_images, n_filters[0])]
        return image, radar_images, scale
